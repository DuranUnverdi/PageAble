...
public class PagerUtil {
    ...
    public static Pageable toPageAble(RestPageableRequest restPageableRequest) {
        ...
        if (!isNullOrEmpty(restPageableRequest.getColumnName())) {
            ...
            boolean asc = Boolean.TRUE.equals(restPageableRequest.getAsc());

            Sort sortBy = asc
                    ? Sort.by(Sort.Direction.ASC, restPageableRequest.getColumnName())
                    : Sort.by(Sort.Direction.DESC, restPageableRequest.getColumnName());

            return PageRequest.of(
                    restPageableRequest.getPageNumber(),
                    restPageableRequest.getPageSize(),
                    sortBy
            );
        }
